...
{
  FILE *my_stream;
  char my_filename[] = "WEIGHTS";
  real weight[nframes], www;
  int     j,k,j3,jk3,m,n;
  real    ccc,cth;
  rvec    xj,xk,rr;
  ...
  /* *** GSHMC *** */
  /* Read weights from WEIGHTS */
  my_stream = fopen(my_filename,"r");
  if (my_stream != NULL)
  {
     for (j=0; (j<nframes); j++) 
     {
        fscanf(my_stream,"%lf",&www);
        weight[j] = www;
        printf("weight read from file: %f \n", weight[j]);
     }
     fclose (my_stream);

     /* Multiply the observables by weights */
     for (j=0; (j<nframes); j++)
        c1[j] *= weight[j];
  }
  else
