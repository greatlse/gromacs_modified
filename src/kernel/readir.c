...
#define CHECK(b) _low_check(b,err_buf,wi)
  ...
  if (ir->met == metGSHMC) 
  {
     ...
     sprintf(err_buf, "GSHMC: number of momentum updates must be at least 1\n");
     CHECK(ir->iMomUpd < 1);
     sprintf(err_buf, "GSHMC: number of MD step must be at least 1\n");
     CHECK(ir->iL < 1);
     sprintf(err_buf, "GSHMC: order of shadow hamiltonian must be 4 or 6\n");
     CHECK(ir->iHamOrd != 4 && ir->iHamOrd != 6);
     sprintf(err_buf, "GSHMC: parameter dTempi (canonical ensemble temperature) must be >0\n");
     CHECK(ir->dTempi < 0);
  }

  /* Adaptive optimization scheme STUFF */
  if (ir->eI == eiTWOSADAPT || ir->eI == eiTWOSADAPTdt)
  {
            ...
            warninp_t wi)
{
  ...
  ITYPE ("nr_MD_steps",           ir->iL,         1000);
  ITYPE ("hamiltonian_order",     ir->iHamOrd,    6);
  RTYPE ("canonical_temperature", ir->dTempi,     300);
