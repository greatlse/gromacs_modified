                             ...
                             double dCoeff)
{
    ...
    for(n=start; n<nrend; n++)
    {
        ...
        for(d=0; d<DIM; d++)
        {
            ...
            {
                if(epc != epcANDERSEN)
                    state->v[n][d] = mv1*(mv1*state->v[n][d] + dCoeff*(w_dt*mv2*f[n][d])) + 0.5*accel[ga][d]*dt;
                else
                {
                    /* Andersen Barostat */
              ...
              if (epc != epcANDERSEN)
{
  ...
  if (epc == epcANDERSEN)
  {
     ...
     for(n=start; n<nrend; n++)
     {
        for(d=0; d<DIM; d++)
        {
           state->v[n][d] = state->v[n][d] * (L1/L3);
           xprime[n][d]   = xprime[n][d] * (L3/L1);
        }
     }
  }
                   ...
                   int n_int)
{
    ...
    /* Used for VNI */
    int coeffVel1,coeffVel2,coeffPos;
...
// THIS HAS TO BE DONE
            if (step == 0)
            {
               ...
               coeffVel1 = 2.0*(stepIntegrator) + 2.0; 
            /* Velocities */
            do_update_vv_vel(start,nrend,dt,
                             ekind->tcstat,ekind->grpstat,
                             inputrec->opts.acc,inputrec->opts.nFreeze,inputrec->epc,
